...
public interface Configuration extends Observed, Singleton
{
	 ...
	 * Resets the configuration to the last stored values.
	 * 
	 * This method can be called manually but should also called periodically for updating the current configuration. This is used if the stored configuration
	 * is changed outside the running application.
	 * 
	 * The implementation should call the update() method of the observers of this class if the stored values are different from the current ones.
	 ...
	 * Makes the current values persistant.
	 * 
	 * If one or more current values are changed, then the current values will be stored and the update() method of the observers will be called.
	 * 
